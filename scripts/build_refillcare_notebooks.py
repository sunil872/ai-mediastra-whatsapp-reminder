...
# Common header for all notebooks to ensure sys.path, display, and matplotlib are always available
...
- How do duplicate invoice lines and missing values behave in pharmacy billing data?'''),
    ...
    md_cell('''## 2. Input Data
We setup the execution environment, resolve paths, and inspect the raw sources.'''),
...
We load a representative sample of raw transactions to inspect columns, data types, and missing values.'''),
...
We parse dates using Indian standard `dayfirst=True` (DD/MM/YYYY) and visualize monthly billing distribution.'''),
    ...
    md_cell('''## 5. What This Means
- **`customerId` is the True Identity Entity:** Because multiple family members share a single phone number, `customerId` must define customer identity. Refill histories must never be merged by phone number.
...
Phase 1 established that the data has stable volume (895K+ rows over ~5.8 years) with sufficient repeat purchase density (60K+ multi-purchase timelines) to support an automated refill prediction system.''')
]
...
Phase 2 implements the production data processing pipeline that cleans raw transactions, aggregates duplicate invoice lines into single purchase visits, enriches medicines with active ingredients from the SALT master catalog, and calculates chronological purchase intervals.'''),
    ...
    md_cell('''## 2. Input Data
We load the canonical purchase history dataset generated by `refillcare.data.pipeline`.'''),
...
1. **Customer Sanitization:** Removes noisy punctuation (e.g. `,ALATHI` $\\rightarrow$ `ALATHI`).
...
display(history_df[cols_to_show].head(8))'''),
    ...
    md_cell('''## 4. Results & Visualizations
We analyze the distribution of refill intervals across all patient histories.'''),
...
Phase 3 formulates the supervised machine learning target ($y_i = t_{i+1} - t_i$) and engineers 36 backward-looking features strictly without lookahead data leakage.'''),
    ...
    md_cell('''## 2. Input Data
We load the partitioned supervised datasets (`train.parquet`, `validation.parquet`, `test.parquet`).'''),
...
print("Generated all 4 notebooks successfully.")
